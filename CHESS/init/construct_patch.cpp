...
void construct_patch(struct patch_object * patch,struct command_line_object * command_line,int rows,int cols,char *filename,char *prefix)
{
	//defaults filename
	...
	channel = (struct channel_object **) calloc(rows*cols, sizeof(struct channel_object));
	for (p = 0; p < rows*cols; p++){
		channel[p] = (struct channel_object *) malloc(sizeof(struct channel_object));
		if (channel[p] == NULL){
			printf("out of memory for assinging soil defaults in construct_patch.c 1\n");
			return;
		}
